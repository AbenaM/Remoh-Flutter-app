...
class _RegisterPatientState extends State<RegisterPatient> with TickerProviderStateMixin {
  ...
  num _age; 
  String _gender; 
  String _relative_name;
  String _relative_number;
  ...
  _formUI() {
    return new Column(
      children: <Widget>[
        ...
        _showAgeInput(),
        _showGenderInput(),
        _showRelativeNameInput(),
        _showRelativePhoneInput(),        
        _showSignUpButton(),
        _showLogin(),
        Utility.loader(load)
      ]
  ...
  Widget _showGenderInput() {
    return Padding(
      ...
      child: new DropdownButtonFormField(
        items: <String>['Male', 'Female', 'Prefer not to mention']
        ...
        validator: (value) =>
            value.isEmpty ? 'Kindly select your gender' : null,
  ...
  Widget _showAgeInput() {
    return Padding(
      ...
      child: new TextFormField(
        ...
        onSaved: (value) => _age =  num.parse(value),
      ),
    );
  }

  Widget _showRelativeNameInput() {
    return Padding(
      padding: const EdgeInsets.fromLTRB(0.0, 10.0, 0.0, 0.0),
      child: new TextFormField(
        maxLines: 1,
        ...
        validator: (value) => value.isEmpty ? 'Relative name can\'t be empty' : null,
        onSaved: (value) => _relative_name = value,
      ),
    );
  }
  ...
  Widget _showRelativePhoneInput() {
    return Padding(
      padding: const EdgeInsets.fromLTRB(0.0, 10.0, 0.0, 0.0),
      child: new TextFormField(
        maxLines: 1,
        ...
        validator: (value) => value.isEmpty ? 'Relative Phone number can\'t be empty' : null,
        onSaved: (value) => _relative_number= value,
      ),
    );
  }
   ...
   registerRequest() async {
    if (_validateAndSave()) {
      try {
        ...
        var response = await auth.patientSignup(
          _email, _password, _firstname, _lastname, _username, _phone_number, _age, _gender, 
          _relative_name, _relative_number
        );
